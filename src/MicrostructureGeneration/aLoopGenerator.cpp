...
aLoopGenerator::aLoopGenerator(const aLoopDensitySpecification& spec, MicrostructureGenerator& mg)
{
    ...
    for (size_t k=0; k<spec.slipSystemIDs.size(); ++k)
    {
        ...
        while(density<targetDensity)
        {
            ...
            const VectorDimD snappedCenter(grain->snapToLattice(L0.cartesian()).cartesian());
            const VectorDimD b(isVL ? grain->slipSystems().at(ssID)->s.cartesian() : -1*grain->slipSystems().at(ssID)->s.cartesian() );
            const VectorDimD loopNormal(isVL? b.normalized() : -1*b.normalized());
            const ReciprocalLatticeDirection<3> reciprocalDir(grain->reciprocalLatticeDirection(loopNormal));
            const int sessilePlaneHeight(reciprocalDir.planeIndexOfPoint(snappedCenter));
            // const LatticePlaneKey sessilePlaneKey(reciprocalDir,sessilePlaneHeight,grainID);
            const LatticePlaneKey sessilePlaneKey(sessilePlaneHeight, reciprocalDir);
            const auto sessilePlane(mg.ddBase.periodicGlidePlaneFactory.getFromKey(sessilePlaneKey));
            
            const VectorDimD R(grain->slipSystems().at(ssID)->unitNormal*radius);
