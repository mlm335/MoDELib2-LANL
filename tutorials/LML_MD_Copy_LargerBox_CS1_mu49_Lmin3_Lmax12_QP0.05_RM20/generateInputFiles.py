...
setInputVariable('inputFiles/'+DDfile,'Nsteps','80000')  # number of simulation steps
setInputVector('inputFiles/'+DDfile,'periodicImageSize',np.array([4, 4, 4]),'number of images for each periodic shift vector') 
setInputVariable('inputFiles/'+DDfile,'EwaldLengthFactor','2') 
# Time Stepping Settings
setInputVariable('inputFiles/'+DDfile,'timeSteppingMethod','fixed') # adaptive or fixed
setInputVariable('inputFiles/'+DDfile,'dtMax','0.5') # 50 fs time step 
...
setInputVariable('inputFiles/'+DDfile,'quadPerLength','0.05')  # number of quadrature points per unit length (in Burgers vector units)
setInputVariable('inputFiles/'+DDfile,'coreSize','2.0')  # number of quadrature points per unit length (in Burgers vector units)
setInputVariable('inputFiles/'+DDfile,'alphaLineTension','3.0') # dimensionless scale factor in for line tension forces
setInputVariable('inputFiles/'+DDfile,'remeshFrequency','20')
setInputVariable('inputFiles/'+DDfile,'Lmin','3')  # min segment length (in Burgers vector units)
setInputVariable('inputFiles/'+DDfile,'Lmax','12')  # max segment length (in Burgers vector units)
...
setInputVariable('inputFiles/'+DDfile,'crossSlipAngle_deg','10')  
...
setInputVariable('inputFiles/'+materialFile,'mu0_SI','49e9')
...
XLength = 338.244807997994e-10/b_SI   # meters/b
YLength = 356.047166313678e-10/b_SI     # meters/b
ZLength = 1764.879600e-10/b_SI    # meters/b
...
# Since the box center is 0,0,0
def frac_pos(L, f):
    print("L: ", L, " f: ", f, ", frac_pos: ", -L/2 + f*L)
    return -L/2 + f*L

p30 = [
    frac_pos(XLength, 0.42),
    frac_pos(YLength, 0.55),
    frac_pos(ZLength, 0.13),
]

p70 = [
    frac_pos(XLength, 0.63),
    frac_pos(YLength, 0.66),
    frac_pos(ZLength, 0.71),
]
...
setInputVector('inputFiles/'+microstructureFile1,'loopRadii_SI',np.array([9e-9,9e-9,9e-9]),'radii of loops in SI')
...
setInputMatrix('inputFiles/'+microstructureFile1,'loopCenters',np.array([[0,0,0],
                                                                         p30,
                                                                         p70]))
